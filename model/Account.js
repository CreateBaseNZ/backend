...
// @FUNC  processChangePasswordCode
// @TYPE  STATICS - PROMISE - ASYNC
// @DESC  Verifies the account
// @ARGU  
AccountSchema.statics.processChangePasswordCode = function (query = {}, save = true) {
  return new Promise(async (resolve, reject) => {
    // CHECK IF ACCOUNT EXIST
    let accountOne;
    try {
      accountOne = await this.findOne(query);
    } catch (error) {
      return reject({ status: "error", content: error });
    }
    if (!accountOne) return reject({ status: "failed", content: "email is not registered" });
    // GENERATE A NEW CODE
    let code;
    ...
    while (generateCode) {
      ...
      let accountTwo;
      try {
        accountTwo = await this.findOne({ "changePassword.code": code });
      } catch (error) {
        return reject({ status: "error", content: error });
      }
      if (!accountTwo) generateCode = false;
    }
    accountOne.changePassword.code = code;
    // FETCH USER DETAILS
    let customer;
    try {
      customer = await Customer.findOne({ accountId: accountOne._id });
    } catch (error) {
      return reject({ status: "error", content: error });
    }
    // SEND THE EMAIL
    ...
    let message;
    try {
      message = await templateTwo(accountOne, customer);
    } catch (data) {
      return reject(data);
    }
    // Construct mail
    const mail = {
      from: `"CreateBase" <${process.env.EMAIL_ADDRESS}>`,
      to: `"${customer.displayName}" ${accountOne.email}`,
      subject: message.subject,
      text: message.text,
      html: message.html
    };
...
// @DESC  Verifies the account
// @ARGU  
AccountSchema.statics.processChangePassword = function (query = {},
  code = "", password = "", save = true) {
  return new Promise(async (resolve, reject) => {
    let error = {
      errorEmail: "",
      errorCode: "",
    ...
    let account;
    try {
      account = await this.findOne(query);
    } catch (error) {
      return reject({ status: "error", content: error });
    }
    // VALIDATE CODE
    if (!account.changePassword.code) {
      ...
      return reject({ status: "failed", content: error });
    }
...
const templateTwo = (account, customer) => {
  return new Promise(async (resolve, reject) => {
    // Create the Subject
