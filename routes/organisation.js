...
// @route     POST /organisation/create-join-email
// @desc
// @access    Backend
router.post("/organisation/create-join-email", async (req, res) => {
	// Validate if the PRIVATE_API_KEY match
	if (req.body.PRIVATE_API_KEY !== process.env.PRIVATE_API_KEY) {
		return res.send({ status: "critical error", content: "" });
	}
	...
	for (let i = 0; i < licenses.length; i++) {
		...
		if (license.organisation) {
			if (license.access === "learner") continue;
			// Fetch the profile and account
			let organisation;
			try {
				organisation = await Organisation.findOne({ _id: license.organisation });
			} catch (error) {
				return res.send({ status: "error", content: error });
			}
			let profile;
			...
			if (license.access === "admin") {
				...
				let mail;
				try {
					mail = await email.create(input, "organisation-detail");
				} catch (data) {
					return res.send(data);
				}
				try {
				...
				let mail2;
				try {
					mail2 = await email.create(input2, "new-org-notif", true);
				} catch (data) {
					return res.send(data);
				}
				// Send the verification email
			...
			} else if (license.access === "educator") {
				const input = {
					email: account.email,
